...
// });
// ------------------------------------
// UNIVERSAL MODAL OPEN/CLOSE (no repeating code)
// ------------------------------------

// Open: any element with [data-modal-target="#some-modal-id"]
...
// Close: clicking backdrop closes ONLY that modal
document.querySelectorAll(".modal .backdrop").forEach((bd) => {
  bd.addEventListener("click", () => {
    bd.closest(".modal")?.classList.remove("open");
  });
});

// Close: ESC closes any open modal
document.addEventListener("keydown", (e) => {
  if (e.key !== "Escape") return;
  document.querySelectorAll(".modal.open").forEach((m) => m.classList.remove("open"));
});

// Close: any element with [data-modal-close] closes the closest modal
...
class ControlledCarousel {
  ...
  updateButtons() {
    if (this.prevBtn) this.prevBtn.disabled = this.state.index <= 0;
    if (this.nextBtn) this.nextBtn.disabled = this.state.index >= this.maxIndex();
  }
...
document.querySelectorAll(".carousel").forEach((el) => {
  new ControlledCarousel(el);
});

// ------------------------------------
// ZINE FLIPBOOK (unchanged logic)
// ------------------------------------
const nextPage = document.getElementById("next-page");
const prevPage = document.getElementById("prev-page");
